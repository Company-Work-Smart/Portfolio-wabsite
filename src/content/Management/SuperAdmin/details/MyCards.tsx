...
function MyCards() {
  ...
  const getCard = async () => {
    const res = await http.get('SuperAdminCardPayment');
    setDatasource(res);
  };

  const handleDelete = async (id: string) => {
    await http.delete(`SuperAdminCardPayment/${id}`);
    await getCard();
  };
  const handleAdd = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingCardId) {
        await http.put(`SuperAdminCardPayment/${editingCardId}`, form);
      } else {
        await http.post('SuperAdminCardPayment', form);
      }
      handleDialogClose();
      await getCard();
    } catch (error) {
